...
/*
 * function and data define by myself
 */
//char** args = (char**)malloc(sizeof(char*) * MAXARGS);
void init_argv();
...
int main(int argc, char **argv)
{
    ...
    // init the argv that cmdline should be turned
...
void eval(char *cmdline)
{
    ...
    strcpy(buff, cmdline);
    int state = parseline(buff, argv)==1? BG: FG;
    ...
    //look at cmdline is builtin?
    if (builtin_cmd(argv) == 1)
    {
        //if this is a builtin command, i will just 'return',
...
/*
    return:
    0 if cmdline is fg
    1 if cmdline is bg or cmdline is none
*/
...
void do_bgfg(char **argv)
{
    ...
    if (strcmp("bg", argv[0]) == 0)
    {
        ...
        {
            target->state = BG;
            printf("[%d] (%d) %s\n", target->jid, target->pid, target->cmdline);
...
void sigchld_handler(int sig)
{
    ...
    sigset_t mask_all, prev_all;
    sigfillset(&mask_all);
    while ((pid = waitpid(-1, &status, WNOHANG | WIFSIGNALED)) > 0)
    {
        sigprocmask(SIG_BLOCK, &mask_all, &prev_all);
        struct job_t* target = getjobpid(jobs, pid);

        //wait fg cmd execute
        if(target->pid == (fgpid(jobs))){
            fg_pid = target->pid;
        }
        //exit normally
        if(WIFEXITED(status)){
            printf("Job [%d] (%d) terminated by return/exit %d", target->jid, target->pid, WEXITSTATUS(status));
            deletejob(jobs, target->pid);
        }else if(WIFSIGNALED(status)){
            printf("Job [%d] (%d) terminated by signal %d", target->jid, target->pid, WTERMSIG(status));
            deletejob(jobs, target->pid);
        }else if(WIFSTOPPED(status)){
            printf("Job [%d] (%d) stopped by signal %d", target->jid, target->pid, WSTOPSIG(status));
            target->state = ST;
        }
        sigprocmask(SIG_SETMASK, &prev_all, NULL);
    }
